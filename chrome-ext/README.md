...
- `manifest.json` — MV3, content script targets `valley.redspell.ru/*` with `all_frames: true`, `world: "MAIN"`.
- `iframe-script.js` — generated by `build.js`. Contains the entire `HC_*` stack plus a `postMessage` bridge.
- The script runs in the iframe's MAIN world, so it can hook `gl.drawElements()` directly on the page's WebGL context.
